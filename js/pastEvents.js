...
let eventopasado = eventosPasados(data.events,data.currentDate)
...
pintartarjetasde4en4(eventopasado, carruselprincipal)
console.log(eventopasado);
...
if (checked.length === 0) {
  ...
  nuevoarreglo = filtrarporcheckbox(eventopasado, checked);
...
function eventosPasados(arreglo,fecha ) { 
  let nuevoArreglo = []
  for (let i = 0; i < arreglo.length; i++) {
    if (arreglo[i].date < fecha) {
      nuevoArreglo.push(arreglo[i]);
...
let checked = Array.from(document.querySelectorAll("input[type=checkbox]:checked")).map(checkbox => checkbox.value.toLowerCase());
let nuevoarreglobuscar = filtrarInputBuscar(eventopasado, e.target.value)
// if (checked.length == 0) {
